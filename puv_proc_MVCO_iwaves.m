...
advbfn = fullfile('C:\Users\ssuttles\data\MVCO\10577vecb-cal.nc'); % burstfile name
advsfn = fullfile('C:\Users\ssuttles\data\MVCO\10577vecs-a.nc'); % statistics filename
...
% depth=depc.depth_corrected;
    ...
    %assume no sinkage during experiment and
    %find all of the offsets  
    zoff = ncreadatt(advsfn,'/','ADVsampleVolumeOffset')*10; % need to multiply by 10 because metadata says 0.0149 sand should be appprox 15cm or 0.15 m
    %zr = brangei-zoff; % time series of ADV sample locations
    z_init = ncreadatt(advsfn,'u_1205','initial_instrument_height')
    zr1=z_init-zoff; %single value
    zr=ones(1,length(dn)).*zr1;
    p_z = ncreadatt(advsfn,'P_4023','initial_instrument_height');
...
depth = zp+(P_4023(gb)); % time series of depth [decibars ~= meters]
...
% for n=nt1:nt2
   if(~isnan(depth(n)))
      ...
      jtb = double(ncread(advbfn,'time',[n],[1]))+double(ncread(advbfn,'time2',[n],[1]))/(3600*24*1000); 
