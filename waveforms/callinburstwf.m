...
load C:\Users\ssuttles\data\FireIsland\analysis\site3\mat\puv_proc_FI_iwaves2 raw depth fs zr zp dn
projstr='Fire Island 2014'
...
depc=load('C:\Users\ssuttles\data\FireIsland\analysis\site3\mat\9917advs_depth_corrected.mat');
h(:)=depc.depth_corrected;

% load vspec data of uhat and Tr 
load('C:\Users\ssuttles\data\FireIsland\analysis\site3\mat\vspec_uhat_tr_depc.mat','uhat_wh','Tr_wh')
...
[UBS,PUV,wf,wh,wfr,wfrwt]=mkinburstwf(projstr,bn,datin,whin,filttype,nwf)


save2pdf(sprintf('pdfs\\burstwaveformview_fi14_b%d',bn),gcf,300)
...
[r, phi, Ur] = skewness_params(Hs(bn), Tr_wh(bn), h(bn));
% P=sqrt(1-r^2);
% b=r/(1+P);
% b1=b*cos(phi)/abs(cos(phi));

[T_c, T_t, T_cu, T_tu, umax, umin, RR, beta] = ...
    abreu_points(r, phi, uhat_wh(bn), Tr_wh(bn)); %changes sign of phi in m-fcn

 
t=0:pi/64:2*pi;
wh.T=Tr_wh(bn);
wh.t=t*wh.T/(2*pi);
[wh.u,wh.a,wh.b,wh.phi,wh.R,wh.beta,wh.alpha]=abreu_fstream(t,r,-phi,uhat_wh(bn),Tr_wh(bn),1,2,1); %change sign of phi to correct known issus with skewness_params m-fcn
...
plot(wf(i).t./wf(i).T,wf(i).ub./Uw(i))
hold on
end
...
text(ax4,1e-2,20,'e)')
title(ax0,sprintf('Fire Island 2014, site 3 Burst Waveforms from direct and parameterized methods- burst %d , %s ',bn,datestr(datin.dn)),'fontsize',12)
