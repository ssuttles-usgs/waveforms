...
clear all ; close all ; clc; 
 wh=fullfile('C:\Users\ssuttles\data\FireIsland\analysis\Taran\9921whp-cal.nc'); % statistics filename
...
h_1d(:)=depc.depth_corrected;

%netcdf_load(wh)
ncload(wh);
nt1=1; nt2=2044; 
init_inst_ht=ncreadatt(wh,'/','initial_instrument_height');
Hs(:)=squeeze(wh_4061);
%h_1d(:)=double(hght_18(1,1,:))+double(init_inst_ht); %add instrument height
s(:,:)=double(squeeze(vspec));  
...
hold on 
load('..\mat\skewness_steve_depc.mat','Su_skewness','ur_maj_rot','vr_min_rot',........
...
save('..\mat\vspec_uhat_tr_depc.mat','uhat_wh','Tr_wh','dn_wh')
